...
data class PublicKeyType(
    ...
    val keyBytes: ByteArray = address.substring(prefix.length).decodeBase58()
    ...
    override fun compareTo(other: PublicKeyType): Int {
        return bytesComparator.compare(publicBytes.sha512().ripemd160(), other.publicBytes.sha512().ripemd160())
...
object PublicKeyTypeSerializer : KSerializer<PublicKeyType> {
    ...
    override fun serialize(encoder: Encoder, value: PublicKeyType) {
        if (encoder is IOEncoder) {
            value.ecKey ?: return
            encoder.encodeByteArray(value.publicBytes)
